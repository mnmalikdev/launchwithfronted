...
const Dashboard = () => {
  ...
  return (
    <div className="flex flex-col bg-bgdark w-screen min-h-screen overflow-hidden ">
      <div
        className={`relative flex flex-col w-full  bg-primary px-8 py-2 h-80  ${userData?.coverImageUrl !== "0" ? "bg-cover" : "bg-bgbox"}`}
        style={{
      ...
      >
        ...
        {userData?.coverImageUrl === "0" ? (
          <button onClick={toggleModal} className="flex flex-col justify-center items-center ">
            <img src="/icons/cover_upload.svg" alt="upload-cover" />
            <p className="text-white font-semibold">Add Image</p>
          </button>
        ) : (
          <button onClick={toggleModal} className="flex w-full  h-full justify-end items-end flex-row rounded-full  cursor-pointer">
            <FiEdit
                  ...
                  >
                    <div className={`flex flex-row justify-start items-center gap-8 px-5 ${previewUrls ? `hidden` : `block`} `}>
                      <button className="flex flex-row items-center justify-center gap-2 bg-orangedark w-fit my-4 px-8 py-3 rounded-2xl  md:px-6 md:py-2">
                        ...
                        <p className="text-white font-semibold text-sm">Upload Cover Picture</p>
                      </button>
                    </div>
                  </FileUploader>
                )}
